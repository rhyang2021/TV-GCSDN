function [LLF,likelihoods,Ht, errors]= arma_bekk_mvgarch_likelihoodXY4RepeatTimeVary(parameters, data, weight, window, p, q, kx, ky, Nr, Nl)
% input: paramter vector
%          data      -- time series
%          p         -- ar_order
%         q         -- arch_order
%         Nr, Nl  -- number of repeat experiments and the index of the
...
A = para_armabekk.A;
BX = abs(para_armabekk.BX);
BY = abs(para_armabekk.BY);
    ...
    for j = Nl(i,1) : Nl(i,2)
        if j < Nl(i,1)+m
            Ht(:,:,j) = uncond;
             ...
             for windowid = 1 : size(window,1)
                if window(windowid,1) <= (j-Nl(i,1)+1) && window(windowid,end) >= (j-Nl(i,1)+1)
                    LLF = LLF + likelihoods(j) * weight(windowid);
                    break;
                end
            end
        end
