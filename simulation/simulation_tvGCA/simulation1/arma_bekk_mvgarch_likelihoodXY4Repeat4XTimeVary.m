function  output= arma_bekk_mvgarch_likelihoodXY4Repeat4XTimeVary(parameters, data, weight, window, p, q, kx, ky, Nr, Nl, indexX, indexY)
            ...
            Ht_y(:,:,j) = uncond(indexY,indexY);
             ...
             for windowid = 1 : size(window,1)
                if window(windowid,1) <= (j-Nl(i,1)+1) && window(windowid,end) >= (j-Nl(i,1)+1)
                    LLF_x = LLF_x+likelihoods_x(j)* weight(windowid);
                    LLF_y = LLF_y+likelihoods_y(j)* weight(windowid);                  
                    break;
                end
             end  
        end
