function output  = arma_bekk_mvgarch4Repeat(data,ar_order,bekk_order, Nr, Nl, ARMABEKKoptions)
%% QMLE 
...
% if nargin<=5 || isempty(ARMABEKKoptions)
% %     options=optimset('fminunc');
%     options.Algorithm =  'interior-point'; %'Active-set';%%;%% %'Active-set';% % 'trust-region-reflective'; %%  %
...
% end
% ObjectFunction = @(parameters)arma_bekk_mvgarch_likelihood4Repeat(parameters, data, ar_order, bekk_order, Nr, Nl);
% ConstraintFunction = @(parameters)stationary_constraint(parameters, ar_order, bekk_order, k, 0,0);
...
%%
[loglikelihood,likelihoods,Ht, errors] = arma_bekk_mvgarch_likelihood4Repeat(parameters, data, ar_order, bekk_order, Nr, Nl);
