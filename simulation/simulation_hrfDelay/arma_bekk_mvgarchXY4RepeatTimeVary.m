...
if Inifeasible > 0    
    ...
    %startingparameters = [reshape(newA, k*k*ar_order, 1);vech(newCx);vech(newCy);reshape(newBX1, kx*k*bekk_order,1);reshape(newBY1, ky*k*bekk_order, 1)];
    Inifeasible = stationary_constraint(startingparameters, ar_order, bekk_order, k, kx, ky);
    if Inifeasible > 0   
        newA1 = newA * 0;
        startingparameters = [newA1;vech(newCx);vech(newCy);reshape(newBX, kx*k*bekk_order,1);reshape(newBY, ky*k*bekk_order, 1)];
        %startingparameters = [reshape(newA1, k*k*ar_order, 1);vech(newCx);vech(newCy);reshape(newBX, kx*k*bekk_order,1);reshape(newBY, ky*k*bekk_order, 1)];
        Inifeasible = stationary_constraint(startingparameters, ar_order, bekk_order, k, kx, ky);
        if Inifeasible > 0   
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% if nargin<=13 || isempty(ARMABEKKoptions)
%     options=optimoptions('simulannealbnd');
%     options.AcceptanceFcn=@acceptancesa_new;
%     options.AnnealingFcn=@annealingfast_new;
%     options.PlotFcn={@saplotbestf,@saplotbestx,@saplotf,@saplottemperature};
