function output = mv_grangerarmabekk4RepeatTimeVary(input_data, window, wid, ar_order, bekk_order, indexX, indexY,fd)
...
%% generate the weight matrix, centered at wid
for i = 1 : size(window, 1)
    preweight(i) = weightfun(i, wid);
end
for i = 1 : size(window, 1)
    weight(i) = preweight(i) / sum(preweight);
end

armabekkX = arma_bekk_mvgarch4RepeatTimeVary(data(:,indexX), weight, window, ar_order, bekk_order, Nr, Nl);
armabekkY = arma_bekk_mvgarch4RepeatTimeVary(data(:,indexY), weight, window, ar_order, bekk_order, Nr, Nl);
para_x=armabekkX.parameters;
para_y=armabekkY.parameters;
armabekkXY = arma_bekk_mvgarchXY4RepeatTimeVary(data, weight, window, ar_order,bekk_order, indexX, indexY, Nr, Nl,para_x,para_y);
...
output.LLF_xx=armabekkX.LLF;
output.LLF_yy=armabekkY.LLF;
output.LLF_XY=armabekkXY.LLF;
output.LLF=armabekkX.LLF+armabekkY.LLF;
output.LLF_x=armabekkXY.LLF_x;
output.LLF_y=armabekkXY.LLF_y;
